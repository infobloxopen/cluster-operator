...
func (r *ReconcileCluster) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if instance.ObjectMeta.DeletionTimestamp.IsZero() {
		//If the CR is being requeued but is in the DONE phase, it must need updating
		// if instance.Status.Phase == clusteroperatorv1alpha1.ClusterDone {
		...
		//if the phase is at PENDING here, that means it failed validation and will go through the process again
		// if instance.Status.Phase == clusteroperatorv1alpha1.ClusterPending {
		// 	// Will requeue in five minutes.
